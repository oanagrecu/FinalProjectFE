...
// 			.then((data) => ui.addToCart(data));
// 	}
// };

document.getElementById('${product.id}').on('click', (e) => {
...
const increaseProductQuantity = (product) => {
	...
	const price = $.grep(productsInCart, (el) => {
		return el.id == productId;
	})[0].price;
	...
	$.each(storageData, (i, el) => {
		if (el.id == productId) {
			...
			$(`#${productId}`)
				.find('.price')
				.html(`$${(price * el.itemsNumber).toFixed(2)}`);
			$(`#${productId}-dropdown`)
				.find('.price')
				.html(`$${(price * el.itemsNumber).toFixed(2)}`);
		}
...
const subtractProductQuantity = (product) => {
	...
	let itemsInCart = $.grep(productsInCart, (el) => {
		return el.id == productId;
	})[0].itemsNumber;
